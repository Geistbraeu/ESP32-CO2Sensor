...
namespace webpage {String render() {
    ...
    String html;
        ...
        html += (state.sensorConnected && !state.sensorWarmingUp) ? "" : " warn";
        ...
        if (state.sensorWarmingUp) {
            sensorStatus = "Sensor warm-up: " + String(state.sensorWarmupRemainingSec) + "s remaining";
...
async function refreshLiveData() {
    try {
        ...
        if (co2Value) {
            ...
            co2Value.className = 'card-value accent' + ((data.sensorConnected && !data.sensorWarmingUp) ? '' : ' warn');
        }
        ...
        if (sensorStatus) {
            if (data.sensorWarmingUp) {
                sensorStatus.textContent = `Sensor warm-up: ${data.sensorWarmupRemainingSec || 0}s remaining`;
